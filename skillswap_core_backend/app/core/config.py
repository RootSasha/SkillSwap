...
class Settings(BaseSettings):
    ...
    # ── Dev-режим автентифікації ─────────────────────────────────────────────
    # У звичайному браузері (npm run dev, не Telegram) немає initData взагалі —
    # window.Telegram не існує. Щоб локальна розробка лишалась зручною
    # (як і раніше з DEV_USER на фронтенді), backend може пропускати
    # HMAC-перевірку для одного конкретного, наперед заданого telegram_id.
    #
    # КРИТИЧНО: DEV_MODE_BYPASS_AUTH=true ТІЛЬКИ в локальному .env,
    # НІКОЛИ на production-сервері — інакше будь-хто зможе видавати себе
    # за DEV_TELEGRAM_ID без жодного підпису. На проді явно тримати
    # DEV_MODE_BYPASS_AUTH=false (значення за замовчуванням).
